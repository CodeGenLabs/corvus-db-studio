...
export interface StructureSyncWizardProps {
  onClose: () => void
  onComplete?: () => void
...
const STEPS = [
  ...
  { id: 'diff', title: 'Khác biệt cấu trúc (Schema Diff)' },
  { id: 'preview', title: 'Xem trước DDL (DDL Preview)' },
  { id: 'execution', title: 'Đồng bộ DDL (Execution)' },
]

export function StructureSyncWizard({
  onClose,
  onComplete,
  initialSourceConnId,
}: StructureSyncWizardProps) {
  ...
  const [previewDdl, setPreviewDdl] = useState<string>(
    '-- DDL Đồng bộ cấu trúc tự động sinh:\nALTER TABLE target.customer ADD COLUMN vip_tier INT DEFAULT 0;\nALTER TABLE target.customer MODIFY COLUMN email VARCHAR(255);\nCREATE INDEX idx_customer_email ON target.customer (email);',
  )
  ...
  const handleStartStructureSync = async () => {
    if (!client) return
    try {
      setJobStatus('running')
      const res = (await client.request('job.start', {
        kind: 'sync',
        name: ['Structure Sync:', sourceConnId, '->', targetConnId].join(' '),
        config: {
          mode: 'structure',
          sourceConnId,
          targetConnId,
  ...
  const handleCancelStructureSync = async () => {
  ...
  return (
    ...
    >
      <div style={{ width: 740, height: 520 }}>
        <WizardShell
          title="Đồng bộ cấu trúc bảng (Structure Synchronization Wizard)"
          steps={STEPS}
          currentStepIndex={currentStep}
          onStepChange={setCurrentStep}
          onCancel={onClose}
          onFinish={handleStartStructureSync}
        ...
        >
          ...
          {currentStep === 1 && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              <div style={{ fontSize: 11.5, color: 'var(--text2)' }}>
                Các khác biệt cấu trúc được phát hiện giữa Nguồn và Đích:
              </div>
              <div style={{ maxHeight: 240, overflow: 'auto', border: '1px solid var(--border)', borderRadius: 4, padding: 8, fontSize: 11.5, fontFamily: 'var(--mono)' }}>
                <div style={{ color: 'var(--green)' }}>[+] customer: Thiếu cột vip_tier (INT)</div>
                <div style={{ color: 'var(--amber)' }}>[~] customer: Cột email khác độ dài (VARCHAR(50) → VARCHAR(255))</div>
                <div style={{ color: 'var(--green)' }}>[+] customer: Thiếu chỉ mục idx_customer_email</div>
              </div>
            </div>
          )}
          ...
          {currentStep === 2 && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10, height: '100%' }}>
              <div style={{ fontSize: 11.5, color: 'var(--text2)' }}>
                Xem trước các câu lệnh DDL sẽ được áp dụng lên cơ sở dữ liệu đích:
              </div>
              <textarea
                value={previewDdl}
                onChange={(e) => setPreviewDdl(e.target.value)}
                style={{
                  flex: 1,
                  fontFamily: 'var(--mono)',
                  fontSize: 11,
          ...
          {currentStep === 3 && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10, height: '100%' }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                ...
                {jobStatus === 'running' && (
                  <button
                    onClick={handleCancelStructureSync}
                    style={{
                      height: 24,
                      padding: '0 10px',
              ...
              >
                {jobLogs.length === 0 ? (
                  <span style={{ color: 'var(--text3)' }}>Nhấp Bắt đầu để đồng bộ DDL.</span>
                ) : (
