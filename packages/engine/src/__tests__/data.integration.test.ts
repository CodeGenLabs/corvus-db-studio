...
beforeAll(async () => {
  ...
  readOnlyProfile.host = container.getHost()
  readOnlyProfile.port = container.getPort()

  await vault.set({ kind: 'db-password', ownerId: 'local-owner', connectionId: CONNECTION_ID }, 'corvus')
  await vault.set({ kind: 'db-password', ownerId: 'local-owner', connectionId: READONLY_CONN_ID }, 'corvus')
